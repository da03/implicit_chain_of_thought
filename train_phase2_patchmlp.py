...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, mode, mlps_patch):
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        beam_output = model.generate(
            ...
            zs=[z.repeat_interleave(beam_size, dim=0) for z in zs],
        )
...
def main():
    ...
    mlps_patch.load_state_dict(torch.load('/n/holyscratch01/rush_lab/Users/yuntian/implicit/model_phase0_4_inter_fixq/checkpoint_6_5e-05/mlps.pt'))
    ...
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(mlps.parameters()) + list(model_q.parameters()) + list(mlps_patch.parameters()), lr=args.lr)
    #optimizer = torch.optim.AdamW([sigmas] + list(model.parameters())+list(model_q.parameters()), lr=args.lr)

...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, mlps_q, mode):
    model.eval()
    model_q.eval()
    total = 0
    word_correct = 0
...
def main():
    ...
    print (ptdtype, dtype)
    model_q = AutoModelForCausalLM.from_pretrained(os.path.join(args.qmodel, 'q')).to(device).to(ptdtype)
    print (args.qmodel)
    ...
    mlps_q.load_state_dict(torch.load(os.path.join(args.qmodel, 'mlps.pt')))
    mlps_q.requires_grad = False
    ...
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(mlps.parameters()), lr=args.lr)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, mlps_q, args.mode)

...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlp):
    model.eval()
    model_q.eval()
    total = 0
    word_correct = 0
...
def main():
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlp)
    ...
    for epoch in range(args.epochs):
        save_model(model, mlp, tokenizer, f'models_mimic_fixq_scratch/checkpoint_{epoch}_{args.lr}')

...
def save_model(model, model_q, tokenizer, model_dir):
...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mode):#, mlps_patch):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
...
def main():
    ...
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(model_q.parameters()), lr=args.lr, **extra_args)
    #optimizer = torch.optim.AdamW([sigmas] + list(model.parameters())+list(model_q.parameters()), lr=args.lr)
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, args.mode)#, mlps_patch)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')

...
def save_model(model, model_q, mlps, mixture_components,  rnn, mlps_patch, tokenizer, model_dir):
...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, mode, mlps_patch):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            with ctx:
                ...
                outputs_cot = model_q.forward_zs_feedp_pred_predicttoken(input_ids=input_ids_cot_real, zs=None, first_ids=first_ids, rnn=rnn, mlps=mlps, relevant_tokens=relevant_tokens, mixture_components=mixture_components, phase2=True)
...
def main():
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)
    ...
    if eval_dataloader is not None:
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, eval_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, mlps, mixture_components, rnn, mlps_patch, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                outputs_cot = model_q.forward_zs_feedp_pred_predicttoken(input_ids=input_ids_cot, zs=None, first_ids=first_ids, clone=True, rnn=rnn, mlps=mlps, relevant_tokens=None, mixture_components=mixture_components, phase2=True)
                hidden_state_relevant_list = outputs_cot.zs_p
                #zs = hidden_state_relevant_list
                zs = []
                for i, z in enumerate(hidden_state_relevant_list):

...
def save_model(model, model_q, mlps, rnn, mlps_patch, tokenizer, model_dir):
...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, rnn, mode, mlps_patch):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            with ctx:
                ...
                outputs_cot = model_q.forward_zs_feedp_pred(input_ids=input_ids_cot, zs=None, mlps=mlps, first_ids=first_ids, rnn=rnn, phase2=True)
...
def main():
    ...
    use_fused = True 
    extra_args = dict(fused=True) if use_fused else dict()
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(mlps.parameters()) + list(rnn.parameters()) + list(model_q.parameters()) + list(mlps_patch.parameters()), lr=args.lr, **extra_args)
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode, mlps_patch)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode, mlps_patch)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, mlps, rnn, mlps_patch, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                outputs_cot = model_q.forward_zs_feedp_pred(input_ids=input_ids_cot, zs=None, mlps=mlps, first_ids=first_ids, clone=True, rnn=rnn, phase2=True)
                hidden_state_relevant_list = outputs_cot.zs_p
                #zs = hidden_state_relevant_list
                zs = []
                for i, z in enumerate(hidden_state_relevant_list):
    ...
    #    accuracy, word_accuracy, ppl = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas)
        ...
        print (f"Val. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. Validation Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode, mlps_patch)
        print (f"Test. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. Validation Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")

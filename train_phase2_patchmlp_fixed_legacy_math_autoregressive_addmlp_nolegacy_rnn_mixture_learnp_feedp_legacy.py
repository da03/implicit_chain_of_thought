...
def save_model(model, model_q, mlps, rnn, tokenizer, model_dir):
...
def main():
    ...
    use_fused = True 
    extra_args = dict(fused=True) if use_fused else dict()
    #optimizer = torch.optim.AdamW(list(model.parameters()) + list(mlps.parameters()) + list(model_q.parameters()) + list(mlps_patch.parameters()), lr=args.lr, **extra_args)
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(mlps.parameters()) + list(rnn.parameters()) + list(model_q.parameters()), lr=args.lr, **extra_args)
    #optimizer = torch.optim.AdamW([sigmas] + list(model.parameters())+list(model_q.parameters()), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, mlps, rnn, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')

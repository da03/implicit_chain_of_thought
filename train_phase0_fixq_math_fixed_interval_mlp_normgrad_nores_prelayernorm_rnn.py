...
def save_model(model, model_q, rnn, tokenizer, model_dir):
...
def evaluate(layer_norm, model, model_q, dataloader, tokenizer, ctx, sigmas, rnn, mode, residual=False, follow=None, use_max=False, interval_arg=-1, last_id_minus=0, arg_max_new_tokens=300):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            with ctx:
                ...
                assert not residual
                outputs_nocot = model.forward_zs(input_ids=input_ids_nocot, zs=hidden_state_relevant_list_rnn, first_ids=first_ids, residual=residual)
            ...
            #import pdb; pdb.set_trace()
                ...
                beam_output = model.generate(
                    ...
                    zs=[z[i:i+1].repeat_interleave(beam_size, dim=0) if z is not None else z for z in hidden_state_relevant_list_rnn],
                    residual=residual,
                )
...
def main():
    ...
    all_params = list(model.parameters()) + list(rnn.parameters())
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(layer_norm, model, model_q, val_dataloader, tokenizer, ctx, sigmas, rnn, args.mode, args.residual==1, args.follow, use_max=True, interval_arg=args.interval, last_id_minus=args.last_id_minus)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(layer_norm, model, model_q, test_dataloader, tokenizer, ctx, sigmas, rnn, args.mode, args.residual==1, args.follow, use_max=True, interval_arg=args.interval, last_id_minus=args.last_id_minus)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, rnn, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                if args.residual == 1:
                    ...
                    clone = False
                outputs_nocot = model.forward_zs(input_ids=input_ids_nocot, zs=hidden_state_relevant_list_rnn, first_ids=first_ids, residual=args.residual==1, clone=clone)
                #outputs_nocot = model.forward_zs_attn(input_ids=input_ids_nocot, attended_to=hidden_states_cot, attended_to_mask=~mask, first_ids=first_ids, sigmas=sigmas)
                zs_p = outputs_nocot.zs_p
                zs_q = hidden_state_relevant_list
                #zs_q = zs0 #outputs_nocot.zs_q
    ...
    #    accuracy, word_accuracy, ppl = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas)
        ...
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(layer_norm, model, model_q, val_dataloader, tokenizer, ctx, sigmas, rnn, args.mode, args.residual==1, args.follow, interval_arg=args.interval, last_id_minus=args.last_id_minus, arg_max_new_tokens=args.max_new_tokens)
        print (f"Val. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. Validation Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(layer_norm, model, model_q, test_dataloader, tokenizer, ctx, sigmas, rnn, args.mode, args.residual==1, args.follow, interval_arg=args.interval, last_id_minus=args.last_id_minus)
        print (f"Test. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. Test Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")

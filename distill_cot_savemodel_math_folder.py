...
def main():
    ...
    for filename in glob.glob(os.path.join(args.folder, 'src1_*')):
        train_dataset = CoTDataset(tokenizer, filename, 1024)
        ...
        accuracy, word_accuracy, ppl = evaluate(model, train_dataloader, tokenizer, ctx, beam_size, save_data=os.path.join(args.save_data, os.path.basename(filename)))

...
def save_model(model, mlp, tokenizer, model_dir):
    os.makedirs(model_dir, exist_ok=True)
    model.save_pretrained(model_dir)
    tokenizer.save_pretrained(model_dir)
    torch.save(mlp.state_dict(), os.path.join(model_dir, 'mlp.pt'))

def extract_answer(text):
    ans = text.strip().replace(',', '')
    return ans

def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlp, mode):
    model.eval()
    model_q.eval()
    total = 0
    word_correct = 0
...
def main():
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlp, args.mode)
    ...
    for epoch in range(args.epochs):
        save_model(model, mlp, tokenizer, f'models_mimic_fixq_feedp_gptsmall_{args.mode}_{args.model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            if step % args.accumulate == args.accumulate-1:
                ...
                torch.nn.utils.clip_grad_norm_(mlp.parameters(), args.max_grad_norm)

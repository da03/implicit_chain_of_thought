...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas):
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        for z, zp, sigma_i in zip(zs_q, zs_p, sigmas):
            kl += ((z-zp)*(z-zp)).sum() / sigma_i / sigma_i / 2 / labels_nocot[..., 1:].ge(0).sum().item()
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            if step % args.accumulate == args.accumulate-1:
                ...
                torch.nn.utils.clip_grad_norm_([sigmas] + list(model.parameters())+list(model_q.parameters()), 1000)
                #import pdb; pdb.set_trace()
                optimizer.step()

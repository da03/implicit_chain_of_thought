...
def save_model(model, model_q, mlps, mlps_patch, tokenizer, model_dir):
...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, mode, mlps_patch):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            with ctx:
                ...
                for i, hidden_states in enumerate(hidden_states_cot[:-1]):
                    ...
                    hidden_state_relevant = mlps_patch[i](mlps[i](hidden_state_relevant))
...
def main():
    ...
    parser.add_argument('--valtest_path', type=str, default=None)
    ...
    use_fused = True 
    extra_args = dict(fused=True) if use_fused else dict()
    all_params = list(model.parameters()) + list(mlps.parameters()) + list(model_q.parameters()) + list(mlps_patch.parameters())
    ...
    if args.valtest_path is not None:
        valtest_dataset = CoTVAEDataset(tokenizer, args.valtest_path, 1024)
        valtest_dataloader = DataLoader(valtest_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=True)
    else:
        valtest_dataloader = None
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, args.mode, mlps_patch)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, args.mode, mlps_patch)
    print (f"Test. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. Test Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")
    if valtest_dataloader is not None:
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, valtest_dataloader, tokenizer, ctx, sigmas, mlps, args.mode, mlps_patch)
        print (f"ValTest. PPL: {ppl}. Loss: {loss}. PPL0: {ppl_nll}. NLL: {loss_nll}. KL: {loss_kl}. ValTest Accuracy: {accuracy}. Word Accuracy: {word_accuracy}")
    #model.train()
    #model_q.train()
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, mlps, mlps_patch, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                zs0 = []
                for i, hidden_states in enumerate(hidden_states_cot[:-1]):
                    hidden_state_relevant = mlps_patch[i](mlps[i](hidden_states.gather(1, relevant_ids[:,i:(i+1)].unsqueeze(-1).expand(-1, -1, hidden_size)).squeeze(1)))
                    zs0.append(hidden_state_relevant)
                    #hidden_state_relevant_list.append(hidden_state_relevant + torch.randn_like(hidden_state_relevant) * sigmas[i])
                    hidden_state_relevant_list.append(hidden_state_relevant)

...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, rnn, mode):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
...
def main():
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode)

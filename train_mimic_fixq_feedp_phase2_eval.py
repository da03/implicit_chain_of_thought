...
def load_model(mlps, model_dir, device, ptdtype):
    print ('loading', model_dir)
    model = AutoModelForCausalLM.from_pretrained(model_dir).to(device).to(ptdtype)
    mlps.load_state_dict(torch.load(os.path.join(model_dir, 'mlps.pt')))
    model_q_dir = os.path.join(model_dir, 'q')
    print ('loading', model_q_dir)
    model_q = AutoModelForCausalLM.from_pretrained(model_q_dir).to(device).to(ptdtype)
    return model, model_q, mlps
...
def main():
    ...
    model, model_q, mlps = load_model(mlps, args.model, device, ptdtype)
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, args.mode)
    ...
    for epoch in range(args.epochs):
        break
        save_model(model, model_q, mlps, tokenizer, f'models_mimic_fixq_feedp_phase2_postfix_saveq/checkpoint_{epoch}_{args.lr}')

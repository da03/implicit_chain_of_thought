...
def evaluate(softmax_p, softmax_p_temp, mult_p, model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, mode, mlps_patch):
    with torch.no_grad():
        model.eval()
        model_q.eval()
        total = 0
...
def main():
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(args.softmax_p, args.softmax_p_temp, args.mult_p, model, model_q, val_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(args.softmax_p, args.softmax_p_temp, args.mult_p, model, model_q, test_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)
    ...
    if eval_dataloader is not None:
        ppl, loss, ppl_nll, loss_nll, loss_kl, word_accuracy, accuracy = evaluate(args.softmax_p, args.softmax_p_temp, args.mult_p, model, model_q, eval_dataloader, tokenizer, ctx, sigmas, mlps, mixture_components, rnn, args.mode, mlps_patch)

...
def read_examples(filename):
    ...
    with open(filename) as fin:
        for line in fin:
            ...
            input = input.strip()
            items = input.split('*')
            input = ''.join(items[0].strip().split()[::-1]) + ' * ' + ''.join(items[1].strip().split()[::-1])
            cot = cot.strip()
...
def parse_arguments():
    ...
    parser.add_argument("--train_file", type=str, default="../data/5_by_5_mult/train.txt")
    parser.add_argument("--test_file", type=str, default="../data/5_by_5_mult/test_bigbench.txt")
    parser.add_argument("--num_shot", type=int, default=5)
    parser.add_argument('--overwrite_cache', action='store_true')
    parser.add_argument("--cache_file", type=str, default="no_cot/5_by_5_mult/cache.db")

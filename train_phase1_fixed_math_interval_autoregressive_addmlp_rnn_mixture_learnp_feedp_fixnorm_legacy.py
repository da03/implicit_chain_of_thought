...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, rnn, mode, follow=None, mlps_patch=None, interval_arg=-1, mixture_size=-1, feed='p', use='argmin', last_id_minus=0):#, mlps_patch=None):
    assert feed in ['p', 'q']
    if feed == 'p':
            ...
            with ctx:
                ...
                for i, hidden_states in enumerate(hidden_states_cot[:-1]):
                    if follow == 'diagonal' or follow == 'diagonal_orig' or follow == 'first_column' or follow == 'last_column':
                        ...
                        hidden_state_relevant = mlps_patch[i](hidden_states_cot[1].gather(1, relevant_ids[:,i:(i+1)].unsqueeze(-1).expand(-1, -1, hidden_size)).squeeze(1))
                    else:
                        assert False
...
def main():
    ...
    parser.add_argument('--small_init', type=int, default=0)
    ...
    if False and args.small_init == 1:
        print ('SMALL INIT')
        args.lr = 1e-7
    ...
    for setting, dataloader in [('val', val_dataloader), ('test', test_dataloader)]:
        for feed, use in [('q', 'gt'), ('p', 'argmin'), ('p', 'pred')]: #, (), (), ()]:
            ...
            ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy, loss_kls, loss_kls_mixture, loss_kls_mixture_pred, loss_probs_mixture, loss_ranks_mixture, loss_kl_pred, loss_kls_pred = evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas, mlps, rnn, args.mode, args.follow, mlps_patch, interval_arg=args.interval, mixture_size=args.mixture_size, feed=feed, use=use, last_id_minus=args.last_id_minus)#, mlps_patch)

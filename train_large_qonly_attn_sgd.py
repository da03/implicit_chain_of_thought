...
def evaluate(model, model_q, dataloader, tokenizer, ctx, sigmas):
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        for z, zp, sigma_i in zip(zs_q, zs_p, sigmas):
            kl += (z*z).sum() / sigma_i / sigma_i / 2 / labels_nocot[..., 1:].ge(0).sum().item()
...
def main():
    ...
    parameters = [sigmas] + list(model.parameters())+list(model_q.parameters())
    ...
    ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy = evaluate(model, model_q, val_dataloader, tokenizer, ctx, sigmas)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            for z, zp, sigma_i in zip(zs_q, zs_p, sigmas):
                ...
                kl += (z*z).sum() / sigma_i / sigma_i / 2 / total
            ...
            if step % args.accumulate == args.accumulate-1:
                ...
                torch.nn.utils.clip_grad_norm_(list(model.parameters())+list(model_q.parameters()), args.max_grad_norm)
                #import pdb; pdb.set_trace()

...
def save_model(model, model_q, mlps_q, mlps, tokenizer, model_dir):
    os.makedirs(model_dir, exist_ok=True)
    print ('saving', model_dir)
    model.save_pretrained(model_dir)
    tokenizer.save_pretrained(model_dir)
    torch.save(mlps.state_dict(), os.path.join(model_dir, 'mlps.pt'))
    torch.save(mlps_q.state_dict(), os.path.join(model_dir, 'mlps_q.pt'))
...
def evaluate(model, model_q, mlps_q, dataloader, tokenizer, ctx, sigmas, mlps, mode, residual=False):
    model.eval()
    model_q.eval()
    total = 0
    word_correct = 0
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        with ctx:
            ...
            for i, hidden_states in enumerate(hidden_states_cot[:-1]):
                ...
                hidden_state_relevant = mlps_q[i](hidden_state_relevant)
...
def main():
    ...
    mlps_q = nn.ModuleList([nn.Sequential(
    ...
    mlps = nn.ModuleList([nn.Sequential(
             nn.Linear(hidden_size_out, hidden_size_mid),
             nn.ReLU(),
             nn.Linear(hidden_size_mid, hidden_size_out),
             ) for _ in range(num_layers_q)]).to(device).to(ptdtype)
    ...
    #import pdb; pdb.set_trace()
    optimizer = torch.optim.AdamW([sigmas] + list(model.parameters())+list(model_q.parameters())+list(mlps.parameters()) + list(mlps_q.parameters()), lr=args.lr)
    ...
    model.eval()
    model_q.eval()
    ppl, loss, ppl_nll, loss_nll, loss_kl, accuracy = evaluate(model, model_q, mlps_q, val_dataloader, tokenizer, ctx, sigmas, mlps, args.mode, args.residual==1)
    ...
    for epoch in range(args.epochs):
        save_model(model, model_q, mlps_q, mlps, tokenizer, f'{args.save_model}/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                zs0 = []
                for i, hidden_states in enumerate(hidden_states_cot[:-1]):
                    hidden_state_relevant = mlps_q[i](hidden_states.gather(1, relevant_ids[:,i:(i+1)].unsqueeze(-1).expand(-1, -1, hidden_size)).squeeze(1))
                    zs0.append(hidden_state_relevant)
                    hidden_state_relevant_list.append(hidden_state_relevant + torch.randn_like(hidden_state_relevant) * sigmas[i])
            ...
            loss = nll + kl
            weight = min(1, (step / len(train_dataloader) / 10)**2)
            #nll.div(args.accumulate).backward()
            (nll+kl*weight).div(args.accumulate).backward()
            #import pdb; pdb.set_trace()
            ...
            if step % args.accumulate == args.accumulate-1:
                ...
                torch.nn.utils.clip_grad_norm_(mlps_q.parameters(), args.max_grad_norm)
            ...
            if step % 100 == 0:
                print (f"Step: {step}. PPL: {ppl}. Loss: {loss}. PPL0: {ppl0}. NLL: {nll}. KL: {kl}. Accuracy: {accuracy}. Weight: {weight}")
                print (sigmas)
                sys.stdout.flush()
            step += 1

...
def main():
    ...
    #import pdb; pdb.set_trace()
    optimizer = torch.optim.AdamW([sigmas] + list(model.parameters())+list(model_q.parameters())+list(mlps.parameters()), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        save_model(model, mlps, tokenizer, f'mult_kl_postfix_{args.mode}_{args.model}_multmlp_anneal/checkpoint_{epoch}_{args.lr}')
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            loss = nll + kl
            weight = min(1, step / len(train_dataloader))
